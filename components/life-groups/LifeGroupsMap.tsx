...
const GROUPS: Group[] = [
  { id: 1,  name: "Brett & Kari's Group", day: "Sunday",    time: "12:30pm", location: "Church Campus",       lat: 32.8754, lng: -117.1425, full: false, onBreak: false, image: "/assets/life-groups/brettkari.png" },
...
function spreadMarkers(groups: Group[]): Array<Group & { displayLat: number; displayLng: number }> {
  ...
  return groups.map(g => {
    ...
    if (siblings.length === 1) return { ...g, displayLat: g.lat, displayLng: g.lng };
    const i = siblings.indexOf(g);
    const angle = (2 * Math.PI * i) / siblings.length - Math.PI / 2;
    return { ...g, displayLat: g.lat + RADIUS * Math.cos(angle), displayLng: g.lng + RADIUS * Math.sin(angle) };
  });
}
    ...
    <>
            ...
            {filtered.map(g => (
              <div
                ...
                style={{
                  ...
                  borderLeft: `4px solid ${g.full ? 'var(--ink-200)' : (DAY_BG[g.day] ?? '#d3ab64')}`,
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    {!g.onBreak && !g.full && (
                      <span style={{ fontSize: '12px', color: 'var(--gold-700)', fontFamily: 'var(--font-semicond)', fontWeight: 700, letterSpacing: '.03em' }}>
                        Join Group →
                      </span>
                    )}
                  </div>
      ...
      {modalGroup && (
        <div
          ...
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.6)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}
        >
          <div style={{ background: 'var(--white)', borderRadius: 'var(--radius-xl)', width: '100%', maxWidth: '480px', overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,.35)', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ background: 'var(--ink-900)', padding: '20px 24px', display: 'flex', gap: '14px', alignItems: 'center' }}>
              ...
              <button onClick={closeModal} aria-label="Close" style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,.6)', fontSize: '24px', cursor: 'pointer', padding: '4px 6px', lineHeight: 1, flexShrink: 0 }}>×</button>
            </div>
            <div style={{ padding: '24px' }}>
              {submitted ? (
                <div style={{ textAlign: 'center', padding: '16px 0 8px' }}>
                  <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: 'var(--gold-100)', color: 'var(--gold-700)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px', fontSize: '26px', fontWeight: 700 }}>✓</div>
                  <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '26px', margin: '0 0 8px', color: 'var(--ink-900)' }}>You&apos;re In!</h3>
                  <p style={{ color: 'var(--text-muted)', marginBottom: '24px', lineHeight: 1.5 }}>We&apos;ll connect you with {modalGroup.name} soon.</p>
                  <button onClick={closeModal} style={{ padding: '12px 32px', background: 'var(--gold-400)', color: 'var(--ink-900)', border: 'none', borderRadius: 'var(--radius-sm)', fontFamily: 'var(--font-semicond)', fontWeight: 700, fontSize: '15px', textTransform: 'uppercase', letterSpacing: '.05em', cursor: 'pointer' }}>Done</button>
                </div>
              ) : (
                <form onSubmit={handleSubmit}>
                  ...
                  {[
                    ...
                    <div key={f.id} style={{ marginBottom: '16px' }}>
                      <label htmlFor={f.id} style={{ display: 'block', fontFamily: 'var(--font-semicond)', fontWeight: 600, fontSize: '12px', textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--ink-500)', marginBottom: '6px' }}>{f.label}</label>
                      <input id={f.id} name={f.id} type={f.type} required={f.required} placeholder={f.placeholder} style={{ width: '100%', padding: '10px 14px', border: '1.5px solid var(--border-default)', borderRadius: 'var(--radius-md)', fontFamily: 'var(--font-sans)', fontSize: '15px', color: 'var(--ink-900)', outline: 'none', boxSizing: 'border-box', background: 'var(--white)' }} />
                    </div>
                  ))}
                  {[
                  ...
                  <button type="submit" disabled={submitting} style={{ width: '100%', marginTop: '10px', padding: '14px', background: 'var(--gold-400)', color: 'var(--ink-900)', border: 'none', borderRadius: 'var(--radius-sm)', fontFamily: 'var(--font-semicond)', fontWeight: 700, fontSize: '16px', textTransform: 'uppercase', letterSpacing: '.06em', cursor: submitting ? 'wait' : 'pointer', opacity: submitting ? 0.7 : 1, transition: 'opacity .15s' }}>
